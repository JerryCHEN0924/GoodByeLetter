  ...
  <ul class="pagination justify-content-center">
    	...
    	<c:choose>
			...
			<c:when test="${page.number > 0}">
				...
				<li class="page-item">
				<a class="page-link" href="${contextRoot }/topGun/vendorDetailsBackup/page?p=${page.number - 0}" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a>
				</li>
		...
		<c:choose>
			...
			<c:when test="${page.number+1 != pageNumber }">
				...
				<li class="page-item">
				<a class="page-link" href="${contextRoot }/topGun/vendorDetailsBackup/page?p=${pageNumber }">${pageNumber }</a>
				</li>
    	...
    	<c:choose>
			...
			<c:when test="${page.number+1 < page.totalPages}">
				...
				<li class="page-item">
					<a class="page-link" href="${contextRoot }/topGun/vendorDetailsBackup/page?p=${page.number + 2}" aria-label="Next"><span aria-hidden="true">&raquo;</span></a>
				</li>
...
<table class="table table-striped table-hover table-bordered border-dark table-success caption-top align-middle">
  ...
  <c:forEach var="vendorDetails" items="${page.content}">
    <tr>
      <th scope="row">${vendorDetails.id}</th>
      <td>${vendorDetails.name}</td>
      <td>${vendorDetails.type}</td>
      <td>${vendorDetails.email}</td>
      <td>${vendorDetails.address}</td>
      <td>${vendorDetails.FK_memberId.memberId}</td>
      <td>${vendorDetails.plevel.plevel}</td>
      <td>
...
<%--       			<a class="btn btn-outline-success" type="button" href="<c:url value='/topGun/vendorDetails/put/${vendorDetails.vendorDetailsId}' />">編輯</a> --%>
				...
				<form:form action="${contextRoot}/topGun/vendorDetailsBackup/put/${vendorDetails.id}" method="get" >
...
<%-- 				</form:form> --%>
				...
				<form:form action="${contextRoot}/topGun/vendorDetailsBackup/delete/${vendorDetails.id}" method="delete" >
